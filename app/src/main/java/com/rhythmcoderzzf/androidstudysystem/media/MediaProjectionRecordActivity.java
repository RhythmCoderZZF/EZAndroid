...
import com.rhythmcoderzzf.androidstudysystem.permission.PermissionUtil2;
import com.rhythmcoderzzf.baselib.BaseActivity;

import java.io.File;

//【#1】MediaRecord录音需要申请android.permission.RECORD_AUDIO危险权限
public class MediaProjectionRecordActivity extends BaseActivity implements View.OnClickListener {
    ...
    ActivityResultLauncher<Intent> startMediaProjection = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
        if (result.getResultCode() == Activity.RESULT_OK) {
            try {
                ...
                recorder.setVideoEncodingBitRate(4 * 1024 * 1024);
                File file = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MOVIES), "luping.mp4");
                file.delete();
                file.createNewFile();
